        ...
        if(userStr){
            ...
            user.forEach((u, index) => {
                ...
                <tr>
                    ...
                    <td>${u.id}</td>
                    <td>${u.name}</td>
                    <td><button class="btn btn-danger delete-todo" data-id=${u.id}>Delete</button></td>
                </tr>
                `
            })
        }
...
const btnDelete = document.querySelectorAl(".delete-todo");
...
const handleDelete = (id: number) => {
    ...
    if(userStr){
        ...
        localStorage.setItem("todoList", JSON.stringify(newUser));
    }
}
